...
public class Tree
{
    ...
    public static void construct(String the_treeStr, List<List<String>> the_genomes)
    {
        ...
        mkNod(tree.get(0), tree_str.substring(0, tree_str.length() - 1));//根据结构字符串创建树结构
        getPrepared(Tree.tree.get(0));//给节点赋予分支长度
        //getPrepared();
    }

    // 该方法传入的参数分别是 树(网络)的根节点、表示树(网络)的 newick格式的字符串。(习惯将二叉树称为树，而将多分枝且父子关系比较复杂的树称为网络)
    // 我此处用到的是二叉树
    public static void mkNod(TreeNode node, String nodeFlag)//根节点，树结构字符串
    {
    ...
    //重新根植这棵树
    public static void reconstruct(TreeNode node) {
