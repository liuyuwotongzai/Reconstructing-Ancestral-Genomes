...
//遍历每一个叶子节点对叶子节点进行编码。参数：传入一个节点的名字，就可以对此节点进行多状态编码
    public static int[] encodeNode(String name) throws IOException {
        ...
        String allGenes[]  = findAllGenes();//先拿到整棵树上的所有基因
...
//遍历所有的节点名称，为每个节点编码，并写入文件中
    public static void encodeAllandWrite() throws IOException {

        String filepath = "./data/pastml.csv";
        String[] allGenomeName = getGenomeName();//所有基因组的名字
        String[] allGenes = findAllGenes();//进化树中所有的基因
        ...
        {
            int[] everyHang = encodeNode(allGenomeName[j]);
