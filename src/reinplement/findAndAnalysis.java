...
public static String findFilePath(String a,int num,String pastmlPath) throws IOException {//参数为找基因a存在文件的路径,文件夹的编号

    String path  = pastmlPath+"newick_Tree_"+num+"_pastml";//结果文件夹的路径
    //将此路径下的文件存放在String数组中
    File file = new File(path);
    File[] fn = file.listFiles();//所有文件的列表
...
//函数，返回某一个基因在祖先中的状态
public static  int analysisState(String a,int num,String pastmlPath) throws IOException//测试成功//参数，某个基因，基因的文件夹编号
{
    String filePath = findFilePath(a,num,pastmlPath);
...
//某一个基因在祖先中的状态清楚了，下面就是所有基因在祖先中的状态
    //求所有基因在祖先中的状态
public static int[] allStates(int num,String path,String pastmlPath) throws IOException {//参数为文件夹的编号,叶子基因组路径，paml输出路径
    String[] allGenes = encode.findAllGenes(path);//拿到所有的基因

    List<Integer> state = new LinkedList<>();//存储所有基因的状态

    for(int i=0;i<allGenes.length;i++)
    {
        state.add(analysisState(allGenes[i],num,pastmlPath));
    }
    //将链表中状态进行转存
